...
def parse_wechat_html(html: str) -> dict:
    ...
    if content_el:
        ...
        logger.warning("No #js_content found, trying fallback extraction methods...")
        
        # Fallback 1: Try var content_noencode = '...' pattern
        cgi_match = re.search(
            r'var\s+(?:content_noencode|ContentNoEncode)\s*=\s*'
            r'(?:window\.a_value_which_never_exists\s*\|\|\s*)?["\'](.+?)["\'];',
            html, re.DOTALL
        )
        if cgi_match:
            content_html = cgi_match.group(1)
            # Unescape common HTML entities
            content_html = content_html.replace("\\'", "'").replace('\\"', '"')
            result["content_html"] = content_html
            logger.info("Fallback 1 (content_noencode) succeeded")
        
        # Fallback 2: Try extracting content from window.cgiDataNew
        if not result["content_html"]:
            ...
            if cgi_script:
                # Try to extract content_noencode from the cgiDataNew object
                content_match = re.search(
                    r'content_noencode\s*:\s*["\'](.+?)["\']',
                    cgi_script.group(1), re.DOTALL
                )
                if content_match:
                    result["content_html"] = content_match.group(1).replace("\\'", "'").replace('\\"', '"')
                    logger.info("Fallback 2 (cgiDataNew.content_noencode) succeeded")
        
        # Fallback 3: Try #js_text_desc for text-sharing articles
        if not result["content_html"]:
            text_desc_el = page.find("#js_text_desc")
            if text_desc_el:
                result["content_html"] = text_desc_el.html_content
                logger.info("Fallback 3 (#js_text_desc) succeeded")
        
        # Fallback 4: Try .rich_media_content (older WeChat article format)
        if not result["content_html"]:
            rich_el = page.find(".rich_media_content")
            if rich_el:
                result["content_html"] = rich_el.html_content
                logger.info("Fallback 4 (.rich_media_content) succeeded")
        
        # Extract images from whatever content we found
        if result["content_html"]:
            # Process images: convert data-src to src
            result["content_html"] = re.sub(
                r'<img([^>]*?)data-src="([^"]+)"',
                r'<img\1src="\2"',
                result["content_html"]
            )
            ...
            img_urls = set()
            for match in re.finditer(r'data-src="(https?://[^"]+)"', result["content_html"]):
                img_urls.add(match.group(1))
            for match in re.finditer(r'src="(https?://mmbiz[^"]+)"', result["content_html"]):
                img_urls.add(match.group(1))
            for match in re.finditer(r'src="//(mmbiz[^"]+)"', result["content_html"]):
                img_urls.add("https://" + match.group(1))
            result["image_urls"] = list(img_urls)
        else:
            logger.warning("All fallback methods failed to extract content")
