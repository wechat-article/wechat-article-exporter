...
### 2025-12-12
- **认证密钥会话管理增强**：添加 4 天会话有效期，响应中包含过期时间和剩余小时数
- **图床上传功能**：实现可配置的图床上传功能，支持自定义 API 地址和批量上传数量
- **自动任务断点续传**：实现任务状态持久化，支持页面刷新后恢复，公众号级别断点续传
- **自动任务控制面板**：新增统一的自动任务管理界面，支持同步、下载、导出三阶段流程控制
- **增量同步优化**：实现基于文章标题的去重逻辑，避免重复同步，提高同步效率
- **数据库性能优化**：解决 N+1 查询问题，添加批量操作接口
- **文档更新**：完善部署指南，添加 MySQL 后端存储配置说明
