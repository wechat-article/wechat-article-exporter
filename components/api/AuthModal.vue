...
async function newLoginSession() {
  ...
  const resp = await $fetch<StartLoginResult>(`/api/public/v1/login/session/${sid}`, { method: 'POST' });
...
async function getQrcode() {
  try {
    ...
    qrcodeSrc.value = `/api/public/v1/login/getqrcode?rnd=${Math.random()}`;
    msg.value = '';
...
async function checkQrcodeStatus() {
  const resp = await $fetch<ScanLoginResult>('/api/public/v1/login/scan', {
    method: 'GET',
  });
  if (resp && resp.base_resp && resp.base_resp.ret === 0) {
    switch (resp.status) {
...
async function bizLogin() {
  try {
    loading.value = true;
    const resp = await $fetch<LoginAccount>('/api/public/v1/login/login', {
      method: 'POST',
    });
    if (resp.err) {
