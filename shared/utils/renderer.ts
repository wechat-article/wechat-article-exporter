...
export async function renderHTMLFromCgiDataNew(cgiData: any) {
  ...
  // 渲染留言
  const commentHTML = await renderComments(cgiData.link);
...
<span>${metadata.readNum || 'view'}</span>
</button>
<!--点赞量-->
<button class="sns_opr_btn">
...
<span>${metadata.oldLikeNum || 'Like'}</span>
</button>
<!--转发量-->
<button class="sns_opr_btn">
...
<span>${metadata.shareNum || 'Share'}</span>
</button>
<!--喜欢量-->
<button class="sns_opr_btn">
...
<span>${metadata.likeNum || 'Popular'}</span>
</button>
<!--评论量-->
<button class="sns_opr_btn">
...
<span>${metadata.commentNum || 'Comment'}</span>
</button>
</div>
</div>`;
}
