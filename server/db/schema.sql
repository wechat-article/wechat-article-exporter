...
CREATE TABLE IF NOT EXISTS `info` (
  `fakeid` VARCHAR(64) NOT NULL PRIMARY KEY COMMENT '公众号唯一标识',
...
CREATE TABLE IF NOT EXISTS `article` (
  `id` VARCHAR(128) NOT NULL PRIMARY KEY COMMENT '主键: fakeid:aid',
  ...
  INDEX `idx_create_time` (`fakeid`, `create_time`),
...
CREATE TABLE IF NOT EXISTS `metadata` (
  ...
  `url_hash` VARCHAR(64) NOT NULL PRIMARY KEY COMMENT 'URL的MD5哈希值',
...
CREATE TABLE IF NOT EXISTS `comment_reply` (
  `id` VARCHAR(256) NOT NULL PRIMARY KEY COMMENT '主键: url_hash:contentID',
  `url` VARCHAR(2048) NOT NULL COMMENT '文章URL',
...
CREATE TABLE IF NOT EXISTS `api_call` (
  ...
  INDEX `idx_call_time` (`account`, `call_time`)
